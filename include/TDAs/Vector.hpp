...
public:
  ...
  void redimensionar(size_t nuevo_tamanio_fisico);
...
template <typename T>
void Vector<T>::redimensionar(size_t nuevo_tamanio_fisico)
{
  T *aux = new T[nuevo_tamanio_fisico];
  ...
  delete[] datos;
  datos = aux;
  tamanio_fisico = nuevo_tamanio_fisico;
}
...
void Vector<T>::alta(T dato)
{
  ...
  {
    size_t nuevo_tamanio_fisico = (tamanio_fisico == 0) ? 1 : tamanio_fisico * 2;
    redimensionar(nuevo_tamanio_fisico);
  }
...
void Vector<T>::alta(T dato, size_t indice)
{
  ...
  {
    throw ExcepcionVector("Índice fuera de rango.");
  }
...
T Vector<T>::baja()
{
  ...
  // Redimensionar si el tamaño lógico es menor o igual a la mitad del tamaño físico.
  if (tamanio_logico <= tamanio_fisico / 2 && tamanio_fisico > 1)
  {
    size_t nuevo_tamanio_fisico = tamanio_fisico / 2;
    redimensionar(nuevo_tamanio_fisico);
  }
